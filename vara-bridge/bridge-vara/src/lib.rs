...
struct VaraBridge {
    /// Minimal amount of balance to submit teleport.
    min_amount: u128,
...
static mut VARA_BRIDGE: Option<VaraBridge> = None;
...
fn static_mut_state() -> &'static mut VaraBridge {
    unsafe { VARA_BRIDGE.get_or_insert(Default::default()) }
}
...
extern fn handle() {
    ...
    let ft: &mut VaraBridge = unsafe { VARA_BRIDGE.get_or_insert(Default::default()) };
    match action {
